...
router.post("/signup", userMiddleware.checkExistingUser, (req, res) => {
  ...
  //     message:
  //       "Invalid Password. Password should between 7 to 15 characters which contain at least one numeric digit and a special character",
  //   });
      ...
      (err, data) => {
        if (err) {
          ...
          email(
            ...
            "registrationSuccess",
            {
              firstName: `${data.fname || data.name}`,
              username: `${req.body.username}`,
            }
          );
          res.status(200).json({
 ...
 *  */
router.get(
  "/generateVarificationCode/:type/:id",
  ...
  (req, res) => {
    ...
    const securityCodeTemplate =
      "<h1>Email varification code is " + securityCode + "</h1>";
    User.Auth.findOneAndUpdate(
      { _id: id },
...
router.put("/register/verification/:type/:id", (req, res) => {
  ...
  User.Register.findById(
    id,
    { emailSecurityCode: 1, phoneSecurityCode: 1 },
    (err, code) => {
      if (err) {
        res.status(400).send(err);
      } else {
            ...
            if (user) {
              ...
              newUser.save((err, data) => {
                if (err) {
                  ...
                  res.status(200).json({
                    token,
                    user: { _id, fname, lname, email, userCategory },
                  });
                }
              });
            }
    ...
    .then((response) => {
      ...
      User.Auth.findOne({ email }).exec(async (err, user) => {
        if (err) {
          ...
          if (user) {
            // User already exists in DB
            const token = jwt.sign({ _id: user._id }, process.env.SecrateKey, {
              expiresIn: "10d",
            });
            const { _id, name, email } = user;
...
router.post(
  ...
  (req, res) => {
    let id = req.params.id;
    User.Auth.findOne({ _id: id }, (err, data) => {
      if (err) {
        res.status(400).send(err);
      } else {
        res.status(200).json({
...
router.get("/searchLocation", userMiddleware.verifyToken, (req, res) => {
  ...
  User.Details.aggregate([{ $match: { ...searchObject } }], (err, data) => {
    if (err) {
      res.status(400).json({
        success: false,
        message: err.message,
...
router.get("/profile", (req, res) => {
  ...
  User.Auth.findOne(obj, (err, data) => {
    if (err) {
      ...
      if (data == null) {
        res.status(401).json({ error: "No find data" });
      } else {
        res.json({
          data: data,
        });
...
router.get("/getprofile/:id", (req, res) => {
  User.Auth.findOne({ _id: req.params.id }, (err, data) => {
    if (err) {
      res.status(400).json({ err });
    } else {
      if (data) {
...
// Update User Profile
router.put("/profile/update/:id", userMiddleware.verifyToken, (req, res) => {
  const id = req.params.id;
  const obj = req.body;
  User.Auth.findOneAndUpdate(
    { _id: id },
